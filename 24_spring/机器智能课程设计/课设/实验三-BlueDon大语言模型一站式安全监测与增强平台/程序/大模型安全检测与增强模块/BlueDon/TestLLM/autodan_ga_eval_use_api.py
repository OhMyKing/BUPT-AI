...
from TestLLM.utils.opt_utils import autodan_sample_control, get_score_autodan_from_api, load_model_and_tokenizer_from_api
...
def generate_str(model, input_str, assistant_role_str_slice, max_tokens = 1024):
...
def autodan_ga_eval_use_api(args = default_args):
    ...
    for i, (g, t, label) in tqdm(enumerate(dataset), total=len(harmful_data.goal[args.start:end])):
        ...
        path = f'./results/attack/attack_log/ga/api_{args.model}_{args.start}_to_{end-1}_{args.save_suffix}.json'

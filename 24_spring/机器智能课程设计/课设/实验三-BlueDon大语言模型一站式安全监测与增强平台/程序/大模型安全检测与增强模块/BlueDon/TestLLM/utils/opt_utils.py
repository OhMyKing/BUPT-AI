...
from TestLLM.utils.string_utils import autodan_SuffixManager
from TestLLM.utils.similarity_utils import SentenceEmbeddingSimilarity
...
def load_model_and_tokenizer_from_api(model_name, developer):
    # 加载模型
    criter = SentenceEmbeddingSimilarity()
    model = LLM(model_name=model_name, developer=developer)
    return model, criter
...
def get_score_autodan_from_api(criter, conv_template, instruction, target, model, device,
                               test_controls=None, max_concurrent_requests=5):
    # Convert all test_controls to token ids
    input_list = []
    target_slices = []
    for item in test_controls:
        suffix_manager = autodan_SuffixManager(conv_template=conv_template,
                                               ...
                                               target=target,
                                               adv_string=item)
        input_str = suffix_manager.get_input_str(adv_string=item)
        input_list.append(input_str)
        target_slices.append(suffix_manager._target_str_slice)

    losses = []
    output_str_list = model.mult_generate_str(input_list, max_tokens=16, temperature=0.9, top_p=0.7,
                                              max_concurrent_requests=max_concurrent_requests)
    uppercased_test_prefixes = [word.upper() for word in test_prefixes]
    prefixes_list = test_prefixes+uppercased_test_prefixes
    
    for index, output_str in enumerate(output_str_list):
        if any(prefix in output_str for prefix in prefixes_list):
            loss_tensor = torch.tensor(10.0, dtype=torch.float16, device=device)
        else:
            lose = criter.compute_similarity_loss(output_str, input_str[target_slices[index]])
            loss_tensor = torch.tensor(lose, dtype=torch.float16, device=device)
        losses.append(loss_tensor)

    del input_list, target_slices

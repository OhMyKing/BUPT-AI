...
from TestLLM.utils.opt_utils import get_score_autodan_from_api, autodan_sample_control
from TestLLM.utils.opt_utils import load_model_and_tokenizer_from_api, autodan_sample_control_hga
...
def generate_str(model, input_str, assistant_role_str_slice, max_tokens = 128):
    ...
    output_str = model.generate_str(input_str,
                                    max_tokens = max_tokens,
                                    top_p=0.9,
                                    temperature=0.7)
    return output_str

def check_for_attack_success_from_str(model, input_str, assistant_role_str_slice, test_prefixes):
    gen_str = generate_str(model,
                           input_str,
                           assistant_role_str_slice,
                           max_tokens = 64)
...
def autodan_hga_eval_use_api(args = default_args):
    ...
    model, criter = load_model_and_tokenizer_from_api(developer='zhipu', model_name=args.model)
    ...
    for i, (g, t, label) in tqdm(enumerate(dataset), total=len(harmful_data.goal[args.start:end])):
        ...
        log = log_init()
        info = {"goal": "", "target": "","label":"", "final_suffix": "",
        ...
        for o in range(len(reference)):
            reference[o] = reference[o].replace('[MODEL]', model.model_name)
            reference[o] = reference[o].replace('[KEEPER]', model.developer)
        ...
        for j in range(num_steps):
            with torch.no_grad():
                ...
                losses = get_score_autodan_from_api(
                    criter=criter,
                ...
                is_success, gen_str = check_for_attack_success_from_str(model,
                                                                        suffix_manager.get_input_str(adv_string=adv_suffix),
                                                                        suffix_manager._assistant_role_str_slice,
                                                                        test_prefixes)
                ...
                print(
                    "################################\n"
                    f"当前指令: {i}/{len(harmful_data.goal[args.start:end])}\n"
                    f"指令内容：{g}\n"
                    f"指令标签：{label}\n"
        ...
        if not os.path.exists('./results/attack/attack_log/hga'):
            os.makedirs('./results/attack/attack_log/hga')
        path = f'./results/attack/attack_log/hga/api_{args.model}_{args.start}_to_{end-1}_{args.save_suffix}.json'

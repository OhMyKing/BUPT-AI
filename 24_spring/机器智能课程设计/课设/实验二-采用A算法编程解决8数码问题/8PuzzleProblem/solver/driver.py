...
def reset_globals():
    global nodes_expanded, max_search_depth, max_frontier_size, goal_node, moves, costs
    nodes_expanded = 0
    max_search_depth = 0
    max_frontier_size = 0
    goal_node = State
...
def solver(start_state, end_state, algorithm='A_star'):
    reset_globals()
    global board_len, board_side, initial_state, goal_state

    goal_state = list(end_state)
...
if __name__ == '__main__':
    for method in ['BFS','DFS','A_star','DL']:
        res = solver([0, 8, 7, 6, 5, 4, 3, 2, 1],[1,2,3,4,5,6,7,8,0],method)
        print('使用方法'+method+'需要步数：')
        print(len(res))
        print('具体步骤为：')
        print(res)
        print('\n')

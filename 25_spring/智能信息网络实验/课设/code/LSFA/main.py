...
def main():
    ...
    parser.add_argument("--learning_rate", default=1e-5, required=False, type=float,
                        help="learning rate of LSFL")

    #training
    parser.add_argument('--gpuid', type=int, default=0,
                        help="gpu id (ignored if no GPU available)")
...
def da4mltc(args):
    ...
    def get_device(args):
        ...
        if torch.cuda.is_available():
            ...
            # 检查指定的GPU是否存在
            if args.gpuid < num_gpus:
                device = torch.device(f'cuda:{args.gpuid}')
                # 设置环境变量
                os.environ['CUDA_VISIBLE_DEVICES'] = str(args.gpuid)
                logger.info(f'Using GPU {args.gpuid}: {torch.cuda.get_device_name(args.gpuid)}')
                return device, True
            else:
                logger.warning(f'GPU {args.gpuid} not available, using GPU 0 instead')
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            logger.info(f'GPU {i} memory: {props.total_memory / 1024**3:.1f} GB')
    ...
    new_model = new_model.to(args.device)  # 确保新模型也在正确的设备上

...
def evaluate_valid(targets, prediction, mlb, args):
    ...
    def get_ndcg(prediction, targets, mlb, top_K, args):
        mlb = MultiLabelBinarizer(classes=list(range(targets.shape[1])), sparse_output=True)
        mlb.fit(None)
        ...
        ndcg = np.average(dcg / log.cumsum()[np.minimum(targets.sum(axis=-1), top_K).astype(int) - 1])

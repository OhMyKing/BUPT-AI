...
#define IDC_STC_USER             107    // 对话界面的用户名展示
#define IDC_STC_ROOM             108    // 聊天室
#define IDC_STC_FAIL             109    // 登陆失败提示
...
static HWND login_btn, quit_btn; // 按钮句柄
// static HWND soft_keyboard_btn;   // 键盘
static HWND soft_keyboard_btn_up, soft_keyboard_btn_down;   // 键盘句柄

static BITMAP bmp_qq_logo;                         // QQ图标位图
static BITMAP bmp_qq_fail;                         // QQ登陆失败图标    
...
// 按钮变色部分
static HWND last_pressed_btn = NULL;      // 记录最后按下的按钮
static BITMAP* last_btn_normal_bmp = NULL; // 记录按钮的正常状态图片
static int timer_id = 100;                // 计时器ID（需唯一）

static BITMAP bmp_btn_Login;                // 登陆按钮
static BITMAP bmp_btn_Keyboard_1;          // 键盘按钮
static BITMAP bmp_btn_Keyboard_2;          // 按下后的键盘按钮
static BITMAP bmp_btn_Quit;                // 退出按钮
static BITMAP bmp_btn_Send_1;                // 发送按钮（正常）
static BITMAP bmp_btn_Send_2;                // 发送按钮（按下后）

static BITMAP bmp_btn_Password;                // 密码
static BITMAP bmp_btn_Username;                // 用户名
...
static CTRLDATA LoginSuccessDlgCtrl[] = {
    ...
    // 聊天室
     {
        CTRL_STATIC, 
        WS_CHILD | WS_VISIBLE | SS_CENTER | SS_WHITEFRAME, 
        140, 0, 100, 20, 
        IDC_STC_ROOM, 
        "MiniQQ Chatting Room",
        0
    },
...
// // 键盘按下回调函数
// void callback_keyboard_up(HWND hwnd, int id, int nc, DWORD add_data)
// {
//     ShowWindow(soft_keyboard_btn_up,   SW_HIDE);
//     ShowWindow(soft_keyboard_btn_down, SW_SHOW);
//     create_num_keyboard(hwnd);              // 加载键盘
//     printf("Button Keyboard up to down\n");//add your code here  
// }

// void callback_keyboard_down(HWND hwnd, int id, int nc, DWORD add_data)
// {
//     ShowWindow(soft_keyboard_btn_down,   SW_HIDE);
//     ShowWindow(soft_keyboard_btn_up, SW_SHOW);
//     InvalidateRect(hwnd, NULL, TRUE);       // 刷新区域去除键盘
//     printf("Button Keyboard down to up\n");//add your code here  
// }
...
static int LoginFailedDialogProc(HWND hDlg, int message, WPARAM wParam, LPARAM lParam)
{
    switch (message) {
        ...
        case MSG_ERASEBKGND:
        {
            ...
            // 绘制QQ图标
            FillBoxWithBitmap(hdc, 20, 0, 200, 200, &bmp_qq_fail);
...
static void create_controls(HWND hWnd)
{
    ...
    // 键盘按钮
    soft_keyboard_btn_up = CreateWindow(
        CTRL_BUTTON,
        "",
        WS_CHILD | WS_VISIBLE | BS_BITMAP,
    ...
    soft_keyboard_btn_down = CreateWindow(
        CTRL_BUTTON,
        "",
        WS_CHILD | WS_VISIBLE | BS_BITMAP,
        IDC_SOFT_KEYBOARD_BTN,
...
static int MiniQQWinProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        case MSG_CREATE:
            ...
            LoadBitmap(HDC_SCREEN, &bmp_qq_fail, "./sc/jiabeng.bmp");

            // 按钮皮肤
            LoadBitmap(HDC_SCREEN, &bmp_btn_Login,    "./src/Login.bmp"); 
            LoadBitmap(HDC_SCREEN, &bmp_btn_Quit,       "./src/Quit.bmp"); 

            LoadBitmap(HDC_SCREEN, &bmp_btn_Keyboard_1, "./src/Keyboard_1.bmp"); 
            LoadBitmap(HDC_SCREEN, &bmp_btn_Keyboard_2, "./src/Keyboard_2.bmp"); 

            LoadBitmap(HDC_SCREEN, &bmp_btn_Password,    "./src/password.bmp"); 
            LoadBitmap(HDC_SCREEN, &bmp_btn_Username,       "./src/username.bmp"); 
